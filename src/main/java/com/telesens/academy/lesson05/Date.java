...
public class Date {//1a-1
    ...
    public boolean checkData(Date checkedDate) throws IllegalDateException {
        if (!((checkedDate.day <= checkedDate.dayMax)))
            throw new IllegalDateException("Day Date exceded", checkedDate.day);
        if (!(checkedDate.day >= checkedDate.dayMin))
            throw new IllegalDateException("Day Date less than 1", checkedDate.day);
        if (!((checkedDate.month <= checkedDate.monthMax)))
            throw new IllegalDateException("Month Date exceded", checkedDate.month);
        if (!(checkedDate.month >= checkedDate.monthMin))
            throw new IllegalDateException("Month Date less than 1", checkedDate.month);
        if (!(checkedDate.year <= checkedDate.yearMax))
            throw new IllegalDateException("Year Date exceded", checkedDate.year);
        if (!(checkedDate.year >= checkedDate.yearMin))
            throw new IllegalDateException("Year Date less than 1", checkedDate.year);
        return true;
    }
